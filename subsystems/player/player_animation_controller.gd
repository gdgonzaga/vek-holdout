...
## Look lean component fed the camera pitch each frame; its knobs live on that node (auto-resolves if empty)
...
func _ready() -> void:
	...
	if _player:
		...
		if not look_lean:
			look_lean = _player.get_node_or_null("LookLean") as LookLean
...
func _process(delta: float) -> void:
	...
	if look_lean:
		look_lean.apply(_player.get_look_pitch(), delta)
