class_name AStar4WayStrategy
extends PathfindingStrategy

## Standard 4-directional stepped voxel A* pathfinder.
##
## Evaluates 4 horizontal orthogonal directions crossed with vertical dy in
## {+1, 0, -1..-max_drop}. Uses Manhattan distance heuristic on the horizontal plane.
...
func find_path(start_cell: Vector3i, target_cell: Vector3i, context: Dictionary) -> Dictionary:
	...
	while not open.is_empty():
		...
		if explored > max_explored:
			push_warning("AStar4WayStrategy: exceeded %d cells" % max_explored)
...
static func _move_cost(off: Vector3i, jump_up_cost: float, drop_cost_per_cell: float) -> float:
	if off.y > 0:
		return jump_up_cost
	if off.y < 0:
		return drop_cost_per_cell * float(-off.y)
	return 1.0
...
static func _heuristic_multi(a: Vector3i, targets: Array[Vector3i]) -> float:
	var best := INF
	for t in targets:
