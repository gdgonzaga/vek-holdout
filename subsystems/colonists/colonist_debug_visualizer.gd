...
func _ready() -> void:
	...
	# Resolve ColonistAI child if present
	...
	# Fallback search for any NavigationAgent3D in parent's children
...
func _resolve_colonist_state() -> String:
	...
	# Check child StateMachine / FSM nodes if present
...
func _draw_navigation_path() -> void:
	...
	# 2. Fallback to NavigationAgent3D if voxel path is empty
	...
	if not path.is_empty() and path_idx < path.size():
		...
		# Waypoint markers for remaining points
		for i in range(path_idx, path.size()):
			_draw_waypoint_marker(path[i], path_color)

	elif target_pos != Vector3.ZERO and colonist_pos.distance_to(target_pos) > 0.3:
		# No active path, but target is known: draw warning direct line
		_immediate_mesh.surface_begin(Mesh.PRIMITIVE_LINES)
		var direct_color: Color = Color.RED
		_immediate_mesh.surface_set_color(direct_color)
		_immediate_mesh.surface_add_vertex(colonist_pos + Vector3.UP * 0.2)
		_immediate_mesh.surface_set_color(direct_color)
		_immediate_mesh.surface_add_vertex(target_pos + Vector3.UP * 0.2)
		_immediate_mesh.surface_end()
