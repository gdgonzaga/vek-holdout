...
## 5. Extracting the `.res` Mesh Resource in Godot

Godot imports `.glb` files as **`PackedScene`** nodes. However, `ItemDef.mesh` expects a standalone **`Mesh` resource** (`ArrayMesh`).

To convert the `.glb` into a reusable `.res` `Mesh` resource inside Godot:

### Method A: Extracting via the Import Dock (Recommended)
1. In Godot's **FileSystem dock**, select the imported `.glb` file.
2. Open the **Import tab** (located in the top-left panel next to the Scene tab).
3. Expand the **Meshes** section.
4. Locate the **Save to File** property and change it from *Disabled* to **Enabled**.
5. Set the save path (e.g. `res://assets/items/mesh_<item_id>.res`).
6. Click **Reimport** at the bottom of the Import panel.
7. Godot will automatically generate a standalone **`mesh_<item_id>.res`** file in your project.

### Method B: Manual Mesh Extraction from Scene
1. Double-click or right-click the `.glb` file in the FileSystem dock → **New Inherited Scene**.
2. Select the `MeshInstance3D` node inside the newly created scene.
3. In the Inspector panel, right-click the **Mesh** property → select **Save...**.
4. Save the resource as `res://assets/items/mesh_<item_id>.res`.

---

## 6. Assigning to `ItemDef` Resource & Godot Material Setup

### A. Assigning the Mesh Resource
1. Open your target item definition resource in `data/items/<item_id>.tres` (e.g. `data/items/tempered_steel_plate.tres`).
2. Drag the extracted **`mesh_<item_id>.res`** file directly into the **`mesh`** property field of the `ItemDef` inspector.
3. *(Optional)* Adjust `visual_scale` if a specific item requires fine-tuning.
...
3. **ORM Texture (AO, Roughness, Metallic & Specular)**:
   - If using `StandardMaterial3D`, set the channel dropdowns explicitly:
     - **Roughness**: Assign **`T_<item_id>_ORM.png`** → change **Texture Channel** to **`Green`** *(Crucial: Godot defaults to Red; leaving it on Red reads AO instead of Roughness)*. Ensure Roughness slider is set to `1.0`.
     - **Metallic**: For non-metals (wood, cloth, stone), set the **Metallic slider to `0.0`** (or assign `T_<item_id>_ORM.png` with **Texture Channel: `Blue`**).
     - **Specular**: For dry, rough materials like raw wood, dirt, or stone, lower the **Specular slider to `0.1` or `0.2`** (default is `0.5`). This kills harsh white sunlight glare.
     - **Ambient Occlusion**: Enable AO → Assign **`T_<item_id>_ORM.png`** (Texture Channel: **`Red`**, Light Affect: `0.0` for natural PBR lighting, or `0.3 - 0.5` for punchier creases).
   - If using `ORMMaterial3D`:
     - Assign **`T_<item_id>_ORM.png`** directly to the **ORM Texture** slot (automatically maps Red=AO, Green=Roughness, Blue=Metallic).
     - Lower **Specular** under Metallic to `0.1 - 0.2` if the material has unwanted direct sun glare.

### C. Godot Material Troubleshooting Checklist
- [ ] **Mesh looks pitch black or muddy dark in Godot?**
  - **Check Metallic**: Non-metals (wood, plastic, stone) must have **Metallic set to `0.0`** (or Texture Channel: **`Blue`**). Setting Metallic to `1.0` or leaving channel on `Red` treats the object as pure metal with zero diffuse light.
  - **Check Roughness Channel**: Ensure Roughness is reading **`Green`**, not **`Red`**.
  - **Check AO Light Affect**: If AO is too dark, lower `Light Affect` to `0.0 - 0.3`.
  - **Check Environment Lighting**: In the 3D editor top toolbar, click `⋮` next to View → ensure **Preview Sun** and **Preview Environment** are enabled.
- [ ] **Mesh looks too shiny or glossy under direct sunlight?**
  - **Lower Specular**: Under the **Metallic** section, drop the **`Specular`** slider from `0.5` down to **`0.1` or `0.2`** to remove harsh white sunlight glints on dry surfaces.
  - **Check Roughness Multiplier**: Ensure the **Roughness** slider is at **`1.0`** so it fully multiplies the green roughness channel.
