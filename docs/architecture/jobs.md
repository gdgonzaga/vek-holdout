...
- **DeployJobDef** (`data/jobs/deploy_job_def.gd`) — Tactical move order dispatched via `Colony.deploy_colonist`; guides the colonist to assigned world coordinates with `requires_adjacent = false`.
- **FetchEquipmentJobDef** (`data/jobs/fetch_equipment_job_def.gd`) — Automated equipment fulfillment dispatched via `EquipmentAudit`; paths to storage crate stocking desired item and equips it directly upon completion.
