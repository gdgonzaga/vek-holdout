...
**Wild flora extension (built, see [Wild Flora](wild-flora.md#flow-trace-colonist-chopremoval-job)):** every `WildFlora` also gets a `Harvestable` component, and `HarvestJobDef` gained a third branch (alongside the `Growable` farm-plot branch and the generic `HarvestParams` branch above) for `WildFlora` targets — `effective_work_time()` reads `WildFloraDef.chop_work_time`, `complete()` fells via a direct `HealthComponent` hit reusing the real-time chopping pipeline. Marking is bulk via a new area-designation tool (`HarvestBoxController`, key **T**) rather than the single-target E toggle, since flora don't carry `ToggleHarvestAction`. Per-def `WildFloraDef.required_tool_tag` (e.g. `"axe"` for timber, empty for plant removal) routes Colony's job spawn to a second job def, `chop.tres` (same `harvest_job_def.gd` script, `required_equipped_tags = [&"axe"]`), instead of the tool-less `harvest.tres` — a static `JobDef` resource can't vary its tool requirement per target, so the choice happens at spawn time. Per-stage `WildFloraStage.can_chop` (mirroring `can_harvest_fruit`) additionally gates `Harvestable.is_claimable()` and real-time `take_damage()`, so a stage that shouldn't be felled is protected from both paths uniformly.
