...
| `BTActionUseBed` | `bt_action_use_bed.gd` | Interacts with beds to satisfy the rest need and play sleeping animations. |
| `BTActionWander` | `bt_action_wander.gd` | Picks a random walkable point within `radius` for idle movement, then pauses for `wait_duration` seconds at the destination before reporting SUCCESS (so the tree rolls a fresh destination next cycle). |
