...
|---|---|---|
| `map.tscn` / `map.gd` | Scene/Script | The MapRoot — the current game world (`Map`, structural container only — no gameplay logic). Swapped by SceneManager on base↔POI transitions. Holds BlockyGrid (structures-only — the template bakes no blocky generator) + optional SmoothGrid (the initial terrain; new maps default `terrain_gen` to the 50 m deep `data/terrain/default_ground.tres`) + containers for player/colonists/enemies/furniture. Enforces `world_bounds` (AABB volume) and constructs invisible `MapBoundaries` static colliders on Layer 1 (World) to contain actors. Authored POIs have per-map scenes at `data/maps/<id>/map.tscn` (see [Maps](maps.md) subsystem); the pristine template lives at `subsystems/maps/map_template.tscn`. |
| `blocky_grid.gd` | Script | The structures half. Implements `IBlockGrid` (in `build/`); wraps `voxel_tool` get/set + the surface-tagged Godot-physics raycast (see `docs/VOXEL-TOOL-NOTES.md`). Owns block get/set, per-cell HP, the damage surface, bounded VoxelTerrain limits via `world_bounds`, and the blocky terrain's collision layer (2). Does NOT own placement UX (that's Build). |
| `smooth_grid.gd` | Script | The natural-terrain half — BlockyGrid's mirror, different mesher/generator. Bounded VoxelTerrain limits and strata volume bake via `world_bounds`. `carve` (sphere) / `carve_box` (hard per-sample box, F15) / `add_material` edit primitives (F8 semantics; `add_material` is the ONLY smooth-add path and writes the F12 material sidecar), per-position material identity via `get_material_at`/`get_material_def_at` (sidecar → `TerrainStrata` depth rules → `default_material`), cached `height_at` with D4 invalidation, `raycast_to_surface` masked to TerrainSmooth. Opt-in: a `MapDef` without `terrain_gen` makes the node free itself at `_ready` (SceneManager injects the def pre-tree). |
| `terrain_height_sampler.gd` | Script (RefCounted) | The one "natural ground height at column (x, z)" source: the snapped-to-whole-meters heightmap image (the same image the smooth generator meshes) or the noise sampler with the generator's exact params. Built lazily by `SmoothGrid.get_height_sampler()`; immutable after construction, so generator worker threads can read it. `SmoothGrid` (strata depth / pristine height) and `WaterGenerator` (shoreline) both read through it, so water and ground cannot disagree about where the bank is. |
| `water_generator.gd` | Script (VoxelGeneratorScript) | Fills the blocky grid with water cubes. A blocky cell (x, z) spans four ground samples (x, z) (x+1, z) (x, z+1) (x+1, z+1); the column is wet when ANY is below the water top and fills down to the lowest one, so cubes overshoot into the bank and the smooth ground buries their step edges. Water is whole cubes: the surface sits at `floor(water_level)`. Installed by `BlockyGrid` only when the sibling `SmoothGrid`'s `terrain_gen.water_enabled` is true. |
...
| `strata_baker.gd` | Script (RefCounted) | Multi-threaded 3D volumetric texture baker (`Option B2`). Palettizes materials (0 = macro background, 1..15 = ores) and bakes `TerrainStrata` into an `ImageTexture3D` (`FORMAT_R8`) via `WorkerThreadPool` to guarantee 100% spatial lockstep between visuals and mined drops. |
| `strata_bake_result.gd` | Script (RefCounted) | Data container holding the baked `ImageTexture3D`, world bounds, palette maps, and CPU slice query methods. |
| `terrain_texture_arrays.gd` | Script (RefCounted) | Builds the terrain shader's three `Texture2DArray`s (albedo, normal, ORME) plus per-layer tiles and tints from the catalog's `PbrTextureSet`s. Layer 0 = surface band, 1 = deep band, `1 + palette_index` = ore. Layers whose size/format mismatch the neutral set are replaced by neutral and reported, never dropped silently. |
| `block_library.gd` | Script (Resource) | Owns the `VoxelBlockyLibrary` the blocky mesher renders with; maps string block_id ↔ integer library index, and id → `BlockDef`. Enforces stable index locking (legacy IDs 1..13 locked, explicit `fixed_index`, future additions appended) to preserve save compatibility across game version updates. Identifies fluids (`is_fluid`, `is_water`) and bakes the library from `data/blocks/` (recursively scanned via `ContentDirLoader`, see [Overview](overview.md#content-directory-loading); ordering/indexing still happens in `block_library.gd` itself). |
...
| `../data/blocks/` | Data | One `.tres` per block type (scrap, stone, metal, reinforced, water at root; the 9 wood-shape variants under `wood/`). Fluid blocks (`water`) configure `is_fluid = true`, `wading_speed_mult` (the drag a wading body feels; read through `BlockLibrary.get_wading_speed_mult` / `BlockyGrid.get_wading_speed_mult_at`), `collision_enabled = false`, `transparency_index = 1`, and `culls_neighbors_of_same_type = true` with custom spatial shaders. See [Data Schemas](data-schemas.md). |
| `../data/terrain/` | Data | `TerrainGenDef` (generator params + walk slope gate + `water_enabled`/`water_level`; optional `heightmap: Texture2D` switches generation from noise to image — brightness maps across `height_start`..`height_start+height_range`, 1 px = 1 m, Lossless import required) and `materials/TerrainMaterialDef` (identity + mining stats: hp, depth band, vein size, spawn weight, dig yields — visuals via `pbr` (`PbrTextureSet`) and `tiles_per_meter`, see F8/F11 and Mining; identity resolves per-position via the F12 sidecar + strata). `data/mining/dig_tool.tres` carries the dig action's stats. See [Data Schemas](data-schemas.md). |

**Walkability seam (D4):** `MapWiring.hybrid_ground_probe` composes the smooth grid's `height_at` with direct SDF lattice checks (`SmoothGrid.is_solid_at` / `VoxelGridAdapter.is_terrain_at` — a cell counts as carved air only when all 8 of its corner samples read air, so `carve_box`'s one-plane bleed into neighbouring walls stays solid). A cell is standable when either the natural surface passes through it on a walkable slope (<= 45 degrees), plain blocky rules hold, or an **underground excavated tunnel/cavity** provides clear air space with solid floor below and >= 2m vertical head clearance. Buried unexcavated solid cells are cancelled. Shallow water (1 voxel depth on solid ground) is walkable with a +2.0 traversal step cost penalty (3.0x multiplier), while deep water (> 1 voxel depth or standing directly on fluid surface) is impassable. See [Pathfinding & Navigation](pathfinding.md) (design note 8), [Maps](maps.md) `wire_colonists`, and [Colonists](colonists.md) VoxelPathfinder.
...
| `set_block_at(pos: Vector3i, block_id: String, rot_index: int = 0) -> void` | Places a block (optionally rotated); seeds `_hp_by_pos[pos] = def.hp`; emits `block_placed`. |
| `get_world_bounds() -> AABB` / `set_world_bounds(bounds: AABB) -> void` | The playable-volume bounds used to limit the VoxelTerrain and gate placement/mining. |
...
| `volume_bake_span_xz` | `int` | `[export]` World XZ span in meters for the 3D strata volume bake (default 192). |
| `volume_bake_span_y` | `int` | `[export]` World Y vertical span in meters for the 3D strata volume bake (default 64). |
| `volume_min_y` | `int` | `[export]` Minimum world Y coordinate for the 3D strata volume bake (default -36). |
...
| `set_material_catalog(materials: Array) -> void` | Injects the BuildLibrary material catalog; builds the strata and bakes the 3D strata volume texture. No injection = pre-mining behavior (everything answers `default_material`). |
| `get_strata_bake_result() -> StrataBakeResult` | Returns the active 3D strata bake result container. |
| `get_strata_palette() -> Dictionary` | Returns the material id to palette index mapping. |
...
- **Base table** — `0` = air (`VoxelBlockyModelEmpty`); the rest load in a locked, stable-by-contract order (the old terrain-at-1 slot went away with the blocky terrain block — natural ground is exclusively the smooth grid now). Order is **not** raw alphabetical id sort: `_sort_block_defs_stably`/`_get_block_sort_rank` rank each def by an explicit `fixed_index` (if `> 0`, wins outright) else its position in the hardcoded `LEGACY_BLOCK_ORDER` list (the 13 blocks shipped as of the dual-voxel rewrite — currently alphabetical by coincidence, not by rule) else a shared fallback rank shared by every other block, tie-broken by `.tres` file path. Consequently, adding a brand-new block (no `fixed_index`, not in `LEGACY_BLOCK_ORDER`) always appends after every locked/fixed block and never re-orders the table, regardless of its `id`; only two or more new blocks added together are ordered relative to each other by file path. Deterministic across runs. Full authoring guidance: `docs/HOWTO-author-blocks.md` §6.
