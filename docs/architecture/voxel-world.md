...
| `../data/blocks/` | Data | One `.tres` per block type (scrap, stone, metal, reinforced, water at root; the 9 wood-shape variants under `wood/`). Fluid blocks (`water`) configure `is_fluid = true`, `collision_enabled = false`, `transparency_index = 1`, and `culls_neighbors_of_same_type = true` with custom spatial shaders. See [Data Schemas](data-schemas.md). |
