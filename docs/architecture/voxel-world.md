...
| `../data/blocks/` | Data | One `.tres` per block type (wood, scrap, stone, metal, reinforced, wood_stairs, water). Fluid blocks (`water`) configure `is_fluid = true`, `collision_enabled = false`, `transparency_index = 1`, and `culls_neighbors_of_same_type = true` with custom spatial shaders. See [Data Schemas](data-schemas.md). |
