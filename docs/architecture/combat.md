...
- **Drops on Death**: when the `HealthComponent` emits `entity_died`, `EnemyBase._on_entity_died()` rolls `EnemyDef.loot_table` through `LootRoller` and scatters the resulting stacks as `WorldItem`s around the death position before freeing itself. A null `enemy_def` or null `loot_table` drops nothing. Only death drops loot: enemies freed directly (raid cleanup) do not. Details: [Loot](loot.md#flow-trace-enemy-drops-loot-on-death).
...
| `subsystems/combat/enemy_base.gd` | Script | Base for all enemies; extends `CharacterBody3D`. Applies its assigned `enemy_def` (`EnemyDef`, nullable — see `tech-debt.md`) at `_ready()`: HP/durability, move speed, `BTPlayer` tree, moodlets. Forwards damage, manages life cycle (rolls `EnemyDef.loot_table` and spawns drops on death), provides stat/moodlet queries (`get_stat_ratio`, `get_stat_value`, `get_active_moodlets`), and implements the `ICombatSource` contract (`get_combat_action`, `get_attack_range`). |
...
| `data/enemies/enemy_def.gd` | Script (Resource) | `EnemyDef` — one archetype's stats, behavior tree, `attack_params`, and optional `loot_table`. Schema: `data-schemas.md`. |
