...
| `subsystems/combat/enemy_library.gd` | Script (Autoload) | `EnemyLibrary`: indexes `data/enemies/*.tres` `EnemyDef` resources by `id`, mirroring `ItemDB`'s `_load_dir` pattern. |
