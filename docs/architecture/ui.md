...
HUD + all full-screen UIs. Each screen is its own `.tscn` scene; reusable subscenes are standalone scenes in `ui/shared/` (`item_row` implemented; health bar, roster row *(planned)*). Real screens live in `ui/`; several GDD-planned screens (`player_screen`, `colony_screen`, `day_summary`, `game_over`, `settings`) have empty placeholder directories and are marked *(planned)* below.

## Files

| File | Type | Responsibility |
|---|---|---|
| `hud/hud.tscn` / `hud.gd` | Scene/Script | Persistent in-game overlay mounted on the HUDLayer (CanvasLayer 10). Contains the **Crosshair**, the **InstructionsLabel** (see its own row below), the **InteractLabel** (target name + default-action hint under the crosshair), and the **InventoryPanel** side panel (its own scene, next row). Also owns the E-key tap-vs-hold timer (tap → `player.interactor.execute_default_action`, hold → `player.interactor.open_interaction_menu`) and listens to `build_placement_toggled` (hides the crosshair during placement). HP/Durability/Stamina/Breath bars, hotbar, and day counter are **planned** (not yet built). |
...
| `colony_management/colony_management.tscn` | Scene | Full-screen colony overview opened with **Tab** (`SceneManager.open_screen("colony_management")`). Eight tabs: Colony Info, Colonists, Labors, Jobs, Crafting, Storage, Squads, Areas. The **Colonists** tab pairs a roster (`colonist_entry`) with `colonist_details_panel`, a sub-tabbed pane: Overview / **Gear** / Skills & Carry / Debug (Debug is hidden outside debug builds). Gear is `colonist_equipment_panel`: a slot list (`equipment_slot_row`) beside an always-visible item picker (`equipment_picker_row`); see [Equipment](equipment.md#colony-management-gear-ui). Pure helpers `gear_text` (slot names), `gear_picker_model` and `gear_status` own slot naming, picker grouping/sorting and the "why is this target still pending" resolver. The Gear sub-tab opens with a `loadout_strip` (apply or save named loadouts, see [Equipment](equipment.md#loadouts-named-slot-templates)); `squad_card` has an **Apply to squad** loadout row; `loadout_ui` holds their shared wording and dropdown helpers. Other sub-scenes: `labor_cell`, `crafting_station_row`, `storage_container_row` (a crate card: contents as `item_row`s in stable order, priority and accepted items via `StorageSummary`, redrawn from the crate's `inventory_changed` in one coalesced rebuild per frame rather than a polling timer), `job_row`, `squad_card`, `area_card`. Reads/writes via the `Colony` autoload's public methods. |
...
| `dig_box_hud/dig_box_hud.tscn` | Scene | Passive overlay on HUDLayer (CanvasLayer 10) displayed in `Mode.DIG_BOX_DESIGNATION`. Shows controls guide and live box dimensions (`W x H x D`). |
| `area_designation_hud/area_designation_hud.tscn` | Scene | Passive overlay on HUDLayer (CanvasLayer 10) displayed in `Mode.AREA_DESIGNATION`. Shows active tool title (e.g. "CHOP TREES", "PAINT AREA"), active designation stage, and controls guide. |
| `designation_menu/designation_menu.tscn` | Scene | Modal menu opened with **T** for batch flora work orders (Remove Plants, Chop Trees, Forage, Cancel Orders) and persistent Area CRUD (New Area, Paint, Erase). Registers with `UiGate`. |
| `inventory/inventory_panel.tscn` | Scene | Player inventory side panel mounted in `hud.tscn` (hidden until opened) and toggled with **I**. An **Equipped** strip (one `equipped_slot_row` per occupied slot, each with Unequip, or a placeholder) sits above the **Carried** list (one `inventory_item_row` per stack: Equip when `Equipment.resolve_equip_slot` finds a slot, Eat for food, Drop / Drop All). Equipped gear lives in `Equipment`, not the inventory, so it appears only in the strip. Owns its own I / Esc handling (I never opens over another modal) and its UiGate registration; rebuilds only while open, coalescing the signals one equip fires into one deferred refresh and keeping keyboard focus across it (`RowFocus`); a one-line status label explains refusals (no room to unequip or stow, already holding the item). |
| `inventory/inventory_item_row.tscn`, `inventory/equipped_slot_row.tscn` | Scene | Row scenes for the inventory panel (`inventory_item_row` inherits `shared/item_row`). They emit intent signals (`equip_pressed`, `eat_pressed`, `drop_pressed(count)`, `unequip_pressed(slot_id)`) and never call the Player themselves. |
| `storage/storage_panel.tscn` | Scene (`class_name StoragePanel`) | Player↔container transfer panel, instantiated by `OpenStorageAction` on a CanvasLayer when the player opens a piece of furniture with storage. Two columns of `storage_item_row`s (stable display order), each moving 1 / 10 / All toward the other side. A row the destination cannot take (filter or full) is dimmed, its buttons disabled and its tooltip says why; a status line reports a partial or failed move ("Moved 4 of 10 Plank. No room in the crate."); a capacity bar per side tints when 90% full; the header shows the container's priority and filter (`StorageSummary`). The **Options** button emits `configure_requested`, which `OpenStorageAction` answers by opening the filter/priority panel on top (the transfer panel stays open and live-updates when the rules change). Esc closes; **I** and **E** close it too, but only while it is the topmost panel. Closes itself if the container leaves the tree (deconstructed, destroyed, map swap). Refreshes coalesce into one deferred rebuild per frame, and keyboard focus survives each rebuild (`RowFocus`). |
| `storage/storage_item_row.tscn` | Scene (`class_name StorageItemRow`) | Inherited scene of `shared/item_row` plus the 1 / 10 / All buttons. `setup_transfer(def, count, movable, block_reason)` dims and disables the row when `movable` is 0 and shows `block_reason` as the tooltip; 10 is disabled for a stack of 10 or fewer (All covers it). Emits `move_requested(count)`; the panel owns the transfer. |
| `storage/storage_summary.gd` | Script (`class_name StorageSummary`, static) | One-line container description: `describe(storage)` gives "Priority 3 - Accepts anything" or "Priority 2 - Accepts: Plank, Stone, Rock +2 more; tagged: food". Shows both the item whitelist and the tag rule because `StorageInventory.is_item_allowed` admits an item matching either. |
| `storage_filter/storage_filter_panel.tscn` | Scene | Storage options panel (item whitelist and hauling priority, titled "Storage Options" like its interaction-menu entry), instantiated by `ConfigureStorageFilterAction` on a CanvasLayer, or by the transfer panel's Options button. Lists all items (by display name, sorted by it; the raw id is only a tooltip) with search filter and checkboxes to restrict container accepted items. Builds its rows once, when the container is set. A container limited by a tag rule (from its `StorageParams`, read-only here) shows that rule and is never called "Unrestricted" (`StorageSummary.status_text`). |
...
| `shared/item_row.tscn` / `item_row.gd` | Scene/Script (`class_name ItemRow`) | The one item-stack line: icon, display name (`ItemDef.get_display_name()`), count and stack weight. `set_compact(true)` shrinks it for dense read-only lists. `InventoryItemRow` is an inherited scene that adds action buttons; the Colony Management `storage_container_row` uses it for crate contents. |
| `shared/item_stack_order.gd` | Script (`class_name ItemStackOrder`, static) | Stable display order for any list of item stacks: gear (`Equipment.is_gear`), then food, then everything else; within a group by natural-case display name, then id. `sorted_item_ids({item_id: count})` replaces iterating the inventory dictionary directly, whose order changes whenever a stack is removed and re-added. Used by the inventory panel, storage panel, crate cards and the colonist carry list. |
| `shared/row_focus.gd` | Script (`class_name RowFocus`, static) | Keeps keyboard focus across a list rebuild. The panels rebuild their rows on every change, which would drop focus after each Enter. Rows opt in with `set_meta(RowFocus.KEY_META, key)` (an item id, a slot id); `capture(list)` before removing the rows and `restore(list, token)` after adding the new ones put focus back on the same button of the same row, or on the row's first usable button, or on whichever row now sits at that position. Used by the inventory panel and the storage transfer panel. |
| `shared/*.tscn` | Scene *(planned — not yet built)* | Remaining reusable subscenes: health_bar, roster_row, job_log_entry, memorial_entry. |
...
- **Persistent panels** (the inventory panel) register/unregister in their open/close functions.
...
1. Register with UiGate from the panel's script:
   ...
   ```
   That one registration is the whole "stop gameplay input" step: WASD/E/B/etc. die automatically (InputComponent checks the gate) and the cursor shows. Freeing the panel reverses both. Panels that persist in the scene instead of being freed — like the inventory panel — register/unregister in their open/close functions instead of `_ready`/`_exit_tree`.
