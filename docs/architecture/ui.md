...
|---|---|---|
| `hud/hud.tscn` / `hud.gd` | Scene/Script | Persistent in-game overlay mounted on the HUDLayer (CanvasLayer 10). Contains the **Crosshair**, an **Instructions** label (build-placement / build-menu text), the **InteractLabel** (target name + default-action hint under the crosshair), and an **InventoryPanel** side panel toggled with **I**. Also owns the E-key tap-vs-hold timer (tap → `Player.execute_default_action`, hold → `Player.open_interaction_menu`) and listens to `build_placement_toggled` / `build_menu_toggled`. HP/Durability/Stamina/Breath bars, hotbar, and day counter are **planned** (not yet built). |
