...
| `storage/storage_panel.tscn` | Scene | Player↔container transfer panel, instantiated by `OpenStorageAction` on a CanvasLayer when the player opens a piece of furniture with storage. Closes itself if the container leaves the tree (deconstructed, destroyed, map swap). |
| `storage_filter/storage_filter_panel.tscn` | Scene | Storage whitelist configuration panel, instantiated by `ConfigureStorageFilterAction` on a CanvasLayer. Lists all items with search filter and checkboxes to restrict container accepted items. |
