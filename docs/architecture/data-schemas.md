...
Dual-voxel natural-terrain generator params (conversion D2/D4). A `MapDef.terrain_gen` pointing at one of these opts a map into smooth terrain; **null = no smooth grid at all** (the map's `SmoothGrid` frees itself). Since the template dropped its flat blocky generator (2026-08-18), `terrain_gen` **is** a new map's initial terrain — `voxel_paint_plugin._create_map_def` pre-fills it with `data/terrain/default_ground.tres` (50 m deep rolling ground; frequency is a slope budget: max slope ≈ `height_range`·π·`noise_frequency` must stay under the walk gate). Fields: `id`, `display_name`, `noise_seed`, `noise_frequency`, `heightmap: Texture2D` (nullable — when set, generation is image-driven instead of noise: pixel brightness 0..1 maps across `height_start`..`height_start+height_range`, 1 px = 1 m, image center = world origin; PNGs in `data/terrain/` must use **Lossless** import or `get_image()` pixel data is destroyed by VRAM compression), `height_start`, `height_range` (shared by both generator paths — 1:1 with `VoxelGeneratorNoise2D` / `VoxelGeneratorImage`, F8-verified + probed names), and `max_walk_slope_deg` (the D4 slope gate, <= 45, consumed in Phase 3). `SceneManager` injects the def into the `SmoothGrid` before the map enters the tree.

## `data/terrain/materials/<id>.tres` (Resource: `terrain_material_def.gd`) — `TerrainMaterialDef`

Identity + mining stats for a natural material — the smooth-terrain mirror of `BlockDef` (natural ground is exclusively this vocabulary; the blocky grid is structures only). Identity is **per-position at runtime** (terrain_mining/plan.md): authored blobs (editor sculpts, smooth placement, structure stamps) carry their material id in per-block voxel metadata (F12: one Dictionary per 16³ block anchored at the block origin — only the lowest-position entry per block survives saves, so the origin slot is the durable one), and natural ground resolves through `TerrainStrata`'s deterministic depth rules (F13 pristine height; `SmoothGrid.set_material_catalog` injects the catalog where `terrain_gen` flows). The mesher remains a one-look-per-map ceiling — per-voxel texturing is verified non-functional (F14) — so the def carries **indirect** visuals: `color` tints the Decal marker that identifies authored blobs, and `texture` feeds the terrain shader's depth-band look when this material is a band endpoint. Fields: `id`, `display_name`, `color` (visual identity: marker tint, flat-color fallback for band endpoints without a texture), `hp` (break pool — dig duration is `work_time × hp / 100`, so 100 keeps the old hardness-1 feel; the future tool-damage model consumes the same pool per swing, with equipment gating matching on `id` by design — no type enum), `min_depth`/`max_depth` (inclusive band in voxel rows below the pristine generated surface; surface row = 0, stable under digging), `vein_size` (approx blocks per vein cluster → strata noise wavelength; the shipped `rock` uses 60 = province-scale on purpose), `spawn_weight` (relative frequency within a band, any non-negative scale, combined by softmax scoring — 0 = never generates; also picks the shader's deep band endpoint), `yields: Array[ItemAmount]` (what one completed dig of THIS material drops into the digger's inventory), `texture: Texture2D` (triplanar band texture — sampled only for the two band endpoints, GPU-only so the default import is fine), `place_radius` (the sphere one placement adds — also the blob ghost's radius), and `icon: Texture2D` (optional build-palette icon; entries fall back to the menu default). `BuildLibrary` catalogs these separately from BuildableDefs (`is_terrain_material` / `get_terrain_material` / `get_terrain_materials` — not unlock-gated; natural materials are ambient content).

## `data/mining/dig_tool.tres` (Resource: `dig_tool_params.gd`) — `DigToolParams`

Stats for the mining dig action (Phase 5). Fields: `work_time` (seconds before hp/skill scaling — `DigAction` multiplies by the target material's `hp / 100` resolved AT THE DIG POSITION, divides by the actor's mining skill multiplier) and `carve_radius` (the carved sphere's radius; also the ghost sphere's — the preview shows exactly what the carve removes; fixed size in v1, owner decision). Preloaded as `BuildLibrary.DIG_TOOL` and handed to `DigAction.begin(...)` by whatever triggers the dig — a future equipped tool item references (or overrides) the same bundle so the build-menu tool and an LMB tool-equip path share one code path.
