...
| `world_item.gd` | Script (`class_name WorldItem`, extends RigidBody3D) | Physical item drop in the 3D world (Layer 5 interaction). Visual mesh from `ItemDef.mesh`, pickup via interaction, forbidden state (`is_forbidden`), and impulse toss on drop. The node is named by its item id (`plank`, `plank2`), and every item lives under the map's `ItemsLayer` (see Design Notes). |
...
- **Every `WorldItem` lives under the map's `ItemsLayer`.** `WorldItem.spawn_at(tree_or_node, ...)` only uses its first argument to reach the `SceneTree` (a colonist, a crate or the tree itself all work); it never parents the item under that node. The parent is the first node in the `"items_layer"` group, registered by `MapWiring.wire_items` ([Maps](maps.md)). The item's position is set before parenting because `_ready` registers it with Colony at that position. `setup()` names the node by its item id, and the readable-name flag on `add_child` turns a second same-id stack into `plank2` instead of an `@plank@2` auto-name.
- **No map, no spawn.** With no `ItemsLayer` registered `spawn_at` logs an error and returns null; there is no fallback to `current_scene`. Suites that spawn items without a `Map` use `ColonySandbox.items_layer` or `ItemsLayerFixture.add_to(self)` from `test/helpers/`. A null return does not crash any caller, but two paths would lose the items silently: `Player.drop_item` has already removed them from the inventory, and `CraftingStation` has already counted the output as produced. The error log is the only signal, which is acceptable because a running game always has a wired map.
