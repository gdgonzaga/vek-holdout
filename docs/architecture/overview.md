...
| `build_placement_toggled(active: bool)` | player subsystem | BuildController, HUD | Mode layer change |
| `build_menu_toggled(open: bool)` | player subsystem | `InstructionsLabel` | Build menu visibility — drives the `InstructionsLabel` text for the menu state |
