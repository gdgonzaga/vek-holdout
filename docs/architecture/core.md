...
4. **Runtime SQLite redirect** (`_redirect_sqlite_stream`): ensures `user://maps/<id>/` exists, then per `Map.persisted_streams()` (blocky `map.sqlite` + smooth `terrain.sqlite` when the map has one): if the terrain's stream is a `VoxelStreamSQLite` backed by a `res://` path, copies the pristine database to its runtime name under `user://maps/<id>/` (only if the runtime copy doesn't already exist — a missing authored db is fine, the generator is the baseline) and repoints the stream there; if the terrain has **no stream** (the template case), injects a `VoxelStreamSQLite` pointing at the runtime path.
...
6. `MapWiring.wire_build` (adapter→grid + smooth grid, FurnitureLayer→container, BlueprintLayer→container/grid/furniture, `BlueprintPlacementStrategy`→blueprint_layer) + `wire_player` (reparent persistent Player, set spawn from `SpawnHelpers` or def fallback, wire camera/exclude into BuildController, reuse the player's `VoxelViewer`). The player spawn snaps XZ-preserving onto `Map.ground_height_at` surface + 0.1 (the highest of hill/plate — authored Y is a hint; save loads overwrite it afterwards via `_restore_player`).
