...
| `build_library.gd` | Autoload | Global catalog (`id → BuildableDef`) of everything buildable. Recursively loads `data/blocks/`, `data/buildables/`, `data/furniture/` via `ContentDirLoader` (see [Overview](overview.md#content-directory-loading)). Delegates "unlocked" to `RunProgress`; seeds defaults at startup + on `EventBus.run_started`. See Autoloads table. |
| `subsystems/core/ghost_preview.gd` | Script | Shared `MeshInstance3D` preview helper (translucent, green/red validity or amber designation). Used by both `BuildController` and `DigBoxController`. |
...
| `i_block_grid.gd` | Script (interface) | Documentation-only contract: `get_block_at`, `set_block_at`, `remove_block_at`, `is_valid_placement`, `is_ground_supported` (the D3 smooth-placement support check), `raycast_to_voxel` (surface-tagged: `"blocky"`/`"smooth"`/`"body"`; smooth hits carry the pre-derived placement cell — see [Voxel / World](voxel-world.md)), `snap_transform`, `height_at`. Implementations duck-type; they do NOT extend it. |
...
| `smooth_placement_strategy.gd` | Script (`RefCounted`) | Terrain-material strategy (Phase 5): `SmoothGrid.add_material` sphere at the committed origin, radius from the `TerrainMaterialDef`. Wired as `BuildController.smooth_strategy` by MapWiring (null grid on terrain-less maps — commit fails safe). |
...
| `voxel_grid_adapter.gd` | Script (`RefCounted`) | `IBlockGrid` impl wrapping `voxel/blocky_grid.gd`. Adds `is_valid_placement` + `snap_transform` + raycast `exclude` passthrough (for player-body exclusion) + `height_at` passthrough, plus the optional `SmoothGrid` ref (`set_smooth_grid`, wired by MapWiring) behind `is_ground_supported`. |
...
| `../data/blocks/` | Data | `BlockDef` per block type (scrap, stone, metal, reinforced, water at root; the 9 wood-shape variants grouped under `wood/`) plus the baked `voxel_library.tres`. See [Data Schemas](data-schemas.md). |
| `../data/buildables/` | Data | Plain `BuildableDef` (player-placed objects not on the voxel grid — e.g. `pole`). |
| `../data/furniture/` | Data | `FurnitureDef` per furniture type, grouped into `crafting_stations/`, `defense/`, `storage/`, `recreation/`, `lighting/`, `farming/` subfolders (the 3 `WildFloraDef` entries and one test fixture sit at the root, unsorted — different schema / not real content); adds `dimensions: Vector3i` + `action_options: Array[ActionOption]`. Partial (C1) — see [Actions & Interaction](actions.md). |
| `../../ui/build_menu/build_menu.tscn` | Scene | The build menu: titled scrollable list. `BuildMenu.populate()` instances one `build_menu_entry.tscn` per unlocked buildable, plus tool entries (Deconstruct, Dig) and terrain-material entries (`BuildLibrary.get_terrain_materials`); clicking any entry emits `EventBus.buildable_selected(id)`. |
...
2. Target cell via `_placement_cell` — surface-tagged: blocky/body hit = struck voxel + face normal; smooth hit = the pre-derived cell (slope normals aren't axis-aligned, D3). Confirmed valid via `grid_adapter.is_valid_placement(cell)` (cell is air); smooth-hit cells additionally pass `grid_adapter.is_ground_supported(cell)`.
...
2. Because the def is not a `BlockDef`, validity runs via `_is_footprint_free` (rather than the block's single-cell air check); smooth-hit anchors additionally pass `is_ground_supported` on the anchor cell.
...
4. `_try_commit` builds the `Transform3D` and calls `strategy.commit(...)` → `InstantPlacementStrategy.commit` → `FurnitureLayer.spawn(def, anchor, rotation_state.step)`:
   - Instantiates `new_furniture_template.tscn` (a `Furniture` root); assigns `root.def_id = def.id` and `root.def = def` (the runtime back-ref static data is read through). Assigns `def.mesh` to the `Mesh` node and builds a footprint-sized `BoxShape3D` collision on the `BuildBody` (layer 5, Build) + a trimesh body (layer 1, World).
   - When `def is FurnitureDef`: performs a two-pass setup. Pass 1 aggregates `action_options` and active capability options (`cap.collect_action_options()`) onto an `InteractionComponent`. Pass 2 introspects `FurnitureCapability` sub-resources on `def` and invokes their registered factory in `FurnitureLayer._capability_registry` (spawning `StorageInventory`, `CraftingStation`, `TurretComponent`, `LightSourceComponent`, `BedComponent`, or `Growable` + `Harvestable`). Component state saving conforms to the `ICapabilityComponent` protocol (`serialize_state` / `deserialize_state`).
...
**Dig tool (mining, Phase 5):** a second sentinel entry, `BuildLibrary.DIG_ID`. Selecting it routes LMB to `_try_dig()`: the build ray must first hit natural terrain (`surface == "smooth"` — the blocky plate, furniture, and blueprints are not diggable ground; deconstruct likewise ignores smooth hits, D3), and `_physics_process` shows a green sphere ghost (`GhostPreview.show_sphere_at`) of exactly the carve volume — centered half a radius into the surface so the sphere bites a bowl instead of shaving a lens. The click starts a `DigAction` (see [Actions](actions.md)) on the wired `player` — a timed action whose gauge registers with UiGate (absorbing repeat clicks) and whose completion carves via `SmoothGrid.carve`, grants the yields of the material **at the dig position** (resolved before the carve — F12 sidecar for authored ground, strata depth rules for natural ground, `default_material` as fallback; hp scales the gauge as `work_time × hp / 100`) to the player's inventory, and records a `mining` skill use. On terrain-less maps (no live SmoothGrid) the tool inertly shows no ghost.

**Smooth material placement (Phase 5):** terrain materials (`TerrainMaterialDef`, cataloged by `BuildLibrary` from `data/terrain/materials/`) appear in the build palette after the buildables. Selecting one routes `_physics_process` to `_update_material_ghost` — a blob sphere of the def's `place_radius`, centered half a radius OUT along the surface normal (anchors into the ground instead of perching tangent; mirror of the dig's half-radius bite), green/red by `_is_smooth_placement_valid`: no character body inside the sphere (player/colonist), no furniture footprint or blueprint in any cell the blob's AABB touches, and no blocky block engulfed — the blocky grid is built structures only (natural ground is the smooth grid), so a terrain blob never buries a block. LMB commits through `smooth_strategy` (`SmoothPlacementStrategy.add_material`), instant and HP-free. Rotation is ignored (spheres have no yaw).
...
| `smooth_strategy` | `IPlacementStrategy` (duck-typed) | Runtime-wired. `SmoothPlacementStrategy` — terrain materials route here (never `strategy`, which resolves BuildableDefs). |
