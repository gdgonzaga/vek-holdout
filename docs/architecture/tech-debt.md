...
- [~] **Colonist pathfinding split** — A* on voxel grid for colonists, NavigationAgent for enemies. Two pathfinding systems in one game is complexity; validate the split pays off vs. NavAgent-for-everything once both are prototyped. **Half prototyped:** the colonist A* (`voxel_pathfinder.gd`, injected walkability predicate) is wired end-to-end into `ColonistAI` (claim → path → arrive). The enemy `NavigationAgent` half is still pending; the split-payoff verdict stays open until enemies land.
...
- **C9** Schemas for already-referenced folders: `data/labors/`, `data/weapons/`, `data/armor/`. **Partially resolved:** `data/labors/` now has `labor_def.gd` + 5 `LaborDef` instances (schema in [Data Schemas](data-schemas.md)); `data/weapons/` + `data/armor/` are still pending.
