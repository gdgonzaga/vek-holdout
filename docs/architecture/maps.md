...
| `wire_flora(map: Map, def: MapDef) -> PlantSpawner` | Wires vegetation regeneration by instantiating and mounting `PlantSpawner` onto the map when `def.flora_spawns_per_day > 0` and `def.flora_palette` is populated. Connects to `EventBus.hour_ticked` to pace dynamic flora growth and provides initial population via `populate_initial_flora()` on fresh map loads. Returns the `PlantSpawner` instance (or `null`). |
