...
| `map_wiring.gd` (`MapWiring`) | Script (`RefCounted`, static) | Extracted wiring utilities, one `wire_*` per map subsystem — `wire_build`, `wire_mining`, `wire_day_night`, `wire_flora`, `wire_player`, `wire_items`, `wire_colonists`, `wire_enemies` + `wire_enemy_pathfinder`, `wire_raids` — plus the shared walkability/cost machinery they all draw on: the ground probes (`blocky_ground_probe` / `hybrid_ground_probe`, D4), `smooth_stand_hint`, and `_compose_walkability` / `_compose_cell_cost` (world-bounds-enforced predicate + traversal-cost callables handed to Colony and to every enemy's pathfinder). The single source of truth for post-instantiate setup; see the Class Reference below for each function. |
...
| `../subsystems/environment/plant_spawner.gd` | Script (`Node`) | `PlantSpawner` node mounted on active map by `MapWiring.wire_flora()`. Paces periodic vegetation regeneration across day cycles up to `MapDef.flora_spawn_cap` via internal timer, listening to `EventBus.day_rolled_over`, `furniture_placed`, and `furniture_removed`. Also exposes `populate_initial_flora()` for initial catch-up population on fresh map loads. |
...
| Function | Description |
|---|---|
| `wire_items(map: Map) -> Node3D` | Ensures the map's `ItemsLayer` exists (`Map.get_items_container()` creates it on first use, so no map scene is edited) and registers it in `WorldItem.ITEMS_LAYER_GROUP` (`"items_layer"`), which is how `WorldItem.spawn_at` finds where loose items go. Idempotent. `SceneManager._wire_map` calls it first, before anything on the map can spawn an item. |
