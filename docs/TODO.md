...
- [ ] **Structural weak-point targeting** (GDD §17 Raids) — Brawlers attack lowest-HP block in range; Shooters path through lowest-resistance opening (gates → Scrap → damaged). No targeting logic yet; the damage primitive itself is real now (`BTActionBreachVoxel` calls `BlockyGrid.apply_damage()`, per-material HP depletes instead of instant-destroy). Needs a structural-analysis pass for enemy AI to pick *which* block/opening.
...
- [ ] **Colonist look lean** (ARCH player.md, "Look: Facing and Lean") — the player's Chest/Head lean follows camera pitch; colonists have no camera, so their pitch source would be the vertical component of `get_aim_direction()` while they hold a combat target (level otherwise). `colonist.tscn` shares the same humanoid bones. Deferred until the player version was validated in play.
...
- [ ] **Recreation moodlet icon art** (ARCH recreation.md) — `data/moodlets/recreation_moodlet.tres` exists and is correctly wired (`stat_id = &"recreation"`, same threshold bands as `rest_moodlet.tres`) but ships with an empty `icons` array and is deliberately **not** registered in `data/colonists/default_colonist.tres::moodlet_defs`, because the art does not exist. Needs seven gauge frames at `assets/custom/recreation_moodlet/recreation-{100,90,75,50,25,10,0}.png` (mirror `assets/custom/rest_moodlet/`) plus one `assets/custom/activity_icons/recreation.png`. Once the PNGs land: fill `icons`, register the def on the colonist, and add `&"recreation"` to `activity_moodlet.tres::activity_icon_map`.
- [ ] **Need rates and Recreation design coverage in the GDD** (ARCH recreation.md) — the game-hour convention is now used for hunger/rest/recreation, but the GDD still has no quantitative decay/restore numbers for any of the three. Also, the `recreation` need, its furniture capability, and the moodlet all exist in code and data, but `docs/GDD.md` has zero recreation/joy/leisure content. A GDD section should ratify the decay rates, the object archetypes, and how recreation eventually feeds Morale.
...
- [ ] **Flora & Shrub Pathing Costs** — Add colonist/enemy pathfinding cost modifiers or movement slowdown when traversing cells occupied by non-blocking shrubs (`blocks_movement == false`).
