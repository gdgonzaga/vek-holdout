...
- [ ] **Recreation design coverage in the GDD** (ARCH recreation.md) — the `recreation` need, its furniture capability, and the moodlet all exist in code and data, but `docs/GDD.md` has zero recreation/joy/leisure content (only a "Morale — Placeholder" attribute row in §6.2). The implementation is ahead of the design doc; a GDD section should ratify the decay rate, the object archetypes, and how recreation eventually feeds Morale.
