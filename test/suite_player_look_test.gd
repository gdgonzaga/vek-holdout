...
func test_look_lean_follows_look_direction() -> void:
	...
	assert_int(controller._chest_bone_idx).is_greater_equal(0)
	assert_int(controller._head_bone_idx).is_greater_equal(0)

	var skel := controller._skeleton
	var baseline_chest := skel.get_bone_pose_rotation(controller._chest_bone_idx)
	var baseline_head := skel.get_bone_pose_rotation(controller._head_bone_idx)
...
## The look-up and look-down limits clamp independently.
func test_look_lean_clamps_up_and_down_separately() -> void:
	var player := _spawn_player()
	var controller := player.anim_controller
	controller.look_lean_max_up_deg = 10.0
	controller.look_lean_max_down_deg = 25.0

	assert_float(rad_to_deg(controller._clamp_look_pitch(deg_to_rad(60.0)))).is_equal_approx(10.0, 0.001)
	assert_float(rad_to_deg(controller._clamp_look_pitch(deg_to_rad(-60.0)))).is_equal_approx(-25.0, 0.001)
	assert_float(rad_to_deg(controller._clamp_look_pitch(deg_to_rad(5.0)))).is_equal_approx(5.0, 0.001)
...
func test_look_lean_reaches_skinning_after_animation() -> void:
	...
	controller.anim_tree.mixer_applied.connect(func() -> void:
		animated_chest[0] = skel.get_bone_pose_rotation(controller._chest_bone_idx))
	skel.skeleton_updated.connect(func() -> void:
		skinned_chest[0] = skel.get_bone_pose_rotation(controller._chest_bone_idx))

	var pitch := deg_to_rad(30.0)
	player._rig.set_orientation(0.0, pitch)
	controller._smoothed_pitch = pitch
	for _i in range(3):
		await get_tree().process_frame

	var expected_deg := rad_to_deg(pitch) * controller.look_lean_chest_share
...
func _head_forward_offset_after_lean(player: Player, pitch: float, baseline_chest: Quaternion, baseline_head: Quaternion) -> float:
	var controller := player.anim_controller
	var skel := controller._skeleton
	skel.set_bone_pose_rotation(controller._chest_bone_idx, baseline_chest)
	skel.set_bone_pose_rotation(controller._head_bone_idx, baseline_head)
	player._rig.set_orientation(0.0, pitch)
	# A delta of 0.0 leaves the eased pitch where it's set, so the lean is deterministic.
	controller._smoothed_pitch = pitch
	controller._apply_look_lean(0.0)
	var head_world := skel.global_transform * skel.get_bone_global_pose(controller._head_bone_idx).origin
	var hips_world := skel.global_transform * skel.get_bone_global_pose(skel.find_bone("Hips")).origin
	var model_forward := controller.visuals.global_transform.basis.z.normalized()
